...
void ring_pagerank(int id, MPI_Status status, int proc_n, int tag){
    ...
    size_t left, right;     
    unsigned *labels;
    int new_length_active, new_length_active1;
    size_t *visited, *active_vertices, *active_vertices1;
    ...
    active_vertices1 = (size_t *) malloc(tot_num_vertices * sizeof(size_t));
    ...
    while (num_iterations < max_iterations) {
        ...
        for (i = 0; i < num_vertices; i++) {
            ...
            for (size_t ci = offsets[i]; ci < offsets[i+1]; ci++) {
                if (labels[edgesDest[ci]] == num_iterations + 1) {
                    visited[i] = 1;
                    active_vertices[new_length_active] = i + left;
                    new_length_active++;
                    break;
                }
            }
        ...
        for(int sas_i = 0; sas_i < proc_n; sas_i++){
            ...
            if (id%2 == 0) {
                for (i = 0; i < new_length_active; i++)
                    labels[active_vertices[i]] = num_iterations + 2;

                MPI_Isend(&active_vertices[0], new_length_active, MPI_UNSIGNED_LONG, (id+1)%proc_n, tag, MPI_COMM_WORLD, &request);
                MPI_Wait(&request, &status);
                int use_id = (id == 0)? (proc_n-1): (id-1)%proc_n;
                MPI_Irecv(&active_vertices[0], tot_num_vertices/NUM_WORKERS, MPI_UNSIGNED_LONG, use_id, MPI_ANY_TAG, MPI_COMM_WORLD, &request);
                MPI_Wait(&request, &status);
                MPI_Get_count(&status, MPI_UNSIGNED_LONG, &new_length_active);
            }
            else {
                MPI_Irecv(&active_vertices1[0], tot_num_vertices/NUM_WORKERS, MPI_UNSIGNED_LONG,  (id-1)%proc_n, MPI_ANY_TAG, MPI_COMM_WORLD, &request);
                MPI_Wait(&request, &status);
                MPI_Get_count(&status, MPI_UNSIGNED_LONG, &new_length_active1);
                ...
                size_t * swap = active_vertices;
                active_vertices = active_vertices1;
                active_vertices1 = swap;

                size_t tmp = new_length_active;
                new_length_active = new_length_active1;
                new_length_active1 = tmp;
            }
        }
        if( clock_gettime( CLOCK_REALTIME, &stop) == -1 ) { perror( "clock gettime" );}       
        time = (stop.tv_sec - start.tv_sec)+ (double)(stop.tv_nsec - start.tv_nsec)/1e9;
